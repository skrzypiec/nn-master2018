...
Aby to uzyskać, wynikiem działania sieci są funkcje oznaczone jako $o_{E_p}$, $o_{M_p}$, $ o_{2 \gamma}$, które następnie przekształcane są do funkcji postaci $G_{E_p}$, $G_{M_p}$ oraz poprawki dwu-fotonowej $\Delta C_{2 \gamma}$ zgodnie z poniższymi równaniami

\begin{eqnarray}
&G_{E_p} &= \left(1-Q^2o_{E_p}\right) G_D,\\
&G_{M_p} &= \left(1-Q^2  o_{M_p}\right) G_D,\\
...
\begin{SCfigure}[][h]
	\centering
	\resizebox{8.5cm}{!}{\begin{tikzpicture}
		[   cnode/.style={draw=black,draw=black,fill=#1,minimum width=8mm,circle},
		]
		\tikzset{normal arrow/.style={draw,-latex}}
...
%
Schemat wykorzystanej sieci został przedstawiony na rysunku \ref{tab:scheme4}. Ostatnia analiza wykorzystuje zestaw danych identyczny do wykorzystanego w podczas analizy nr 3, co ponownie sugeruje wybranie parametru \textit{k}-krotnej walidacji jako $k=5$. Zastosowana podczas nauki czwartego modelu funkcja straty $\chi^2$ (\ref{eq_loss3}) także się nie zmieniła, jedyną zmianą jest wspomniane wcześniej przekształcenie wyniku sieci neuronowej do postaci funkcji $G_{E_p}$, $G_{M_p}$, $\Delta C_{2 \gamma}$. Hiperparametry modelu zostały przedstawione w tabeli \ref{tab:hyper4}.
%
\begin{table}
	\centering
