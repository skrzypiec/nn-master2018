...
\begin{itemize}
	...
	\item analiza3
	\begin{itemize}
	\item wszystko do opisania
	\end{itemize}
	\item problem puzzle proton size opisac we wstepie
...
%\chapter{Wstęp}
\chapter[\color{red}Wstęp]{Wstęp}

\section[\color{red}Fizyka zagadnienia i cel pracy - bardzo ogólnie]{Fizyka zagadnienia i cel pracy - bardzo ogólnie}

\section[\color{red}Sieci neuronowe, wysokopoziomwy opis, zastosowania - regresja i klasyfikacja - bardzo ogólnie]{Sieci neuronowe, wysokopoziomwy opis, zastosowania - regresja i klasyfikacja - bardzo ogólnie}
...
\section[\color{red}Opis algorytmu uczenia prostej sieci]{Opis algorytmu uczenia prostej sieci}
...
\section[\color{red}Uniwersalne twierdzenie aproksymacyjne (Twierdzenie Cybenki)]{Uniwersalne twierdzenie aproksymacyjne (Twierdzenie Cybenki)}

\subsection[\color{red} Dowód matematyczny]{Dowód matematyczny}
...
\subsection[\color{red}Przedstawienie wizualne działania]{Przedstawienie wizualne działania}
...
\section[\color{red} Problem Bias - Variance]{Problem Bias - Variance}
...
Zakładając rozpraszanie cząstki $\alpha$ w polu siły centralej $\sim \frac{1}{r^2}$, wykorzystując zasadę zachowania momentu pędu oraz energii mechanicznej otrzymujemy wzór na różniczkowy przekrój czynny, którym posługiwał się Rutherford
%
\begin{equation}
...
\end{equation}
%
przy wykorzystujaniu przybliżenia Borna, czyli jednej z metod przybliżonego rozwiązywania równania na stacjonarne stany rozproszeniowe, otrzymujemy wzór Rutherforda na różniczkowy przekrój czynny na rozpraszanie w potencjale kulombowskim $-\frac{\alpha}{r} = -\frac{e^2}{4\pi \epsilon_0}\frac{1}{r}$:
%
\begin{equation}
...
\begin{itemize}
	\item Dla bardzo małej energii elektronu, $\lambda \gg r_p$, gdzie $r_p$ jest promieniem protonu, rozpraszanie jest równoważne rozpraszaniu na punktowej, bezspinowej cząstce.
...
Powyższe równanie bierze jednak pod uwagę oddziaływania elektronu z protonem traktując proton jako obiekt punktowy, przy dostacznie wysokiej energii eletronu proton ujawnia swoje fizyczne wymiary oraz strukturę wewnętrzną. Ich skutkiem jest rozkład ładunku elektrycznego i momentu magnetycznego protonu. Struktura protonu opisywana jest więc przez dwie funkcje postaci, elektryczną $G_{E_p}$ i magnetyczną $G_{M_p}$ które są transformatami Fouriera, odpowiednio rozkładu ładunku elektrycznego i rozkładu momentu magnetycznego protonu.  Zmodyfikowane równanie \ref{eq:mott} nosi nazwę przekroju czynnego Rosenblutha
...
\end{eqnarray*}
gdzie $\mu_p$ to moment magnetyczny protonu. Wartości tych istotnych funkcji są wyznaczane eksperymentalnie. Zmierzony w laboratorium całkowity przekrój czynny rozpraszania można w przybliżeniu jednofotonowym wyrazić formułą:
% 
\begin{equation}
\begin{split}
...
\subsubsection{Transfer polaryzacji}
...
\subsubsection{Poprawka dwufotonowa (TPE)}
...
Obliczenie poprawki jest trudne i zależne od modelu, wiedząc jednak, że amplituda rozpraszania musi spełniać ogólne zasady symetrii możemy założyć, że 

\begin{equation}
\sigma_R \rightarrow \sigma_R + \delta_{TPE}\left(Q^2, \epsilon\right)
\end{equation}

W jednej z przedstawionej poniżej analiz wpływ poprawki dwufotonowej zostanie uwzględniony. Dokładne oszacowanie przebiegów funkcji postaci $G_{E_p}$ i $G_{M_p}$ jest niezwykle ważne, poniważ może posłużyć do oszacowania promienia protonu, który wyraża się wzorem:
...
\begin{figure}
	...
	\begin{subfigure}[b]{1\textwidth}
		\includegraphics[width=1\linewidth]{ge_.eps}
		\caption{}
		\label{fig:ge1} 
	\end{subfigure}
	
	\begin{subfigure}[b]{1\textwidth}
		\includegraphics[width=1\linewidth]{gm_.eps}
		\caption{}
		\label{fig:gm1}
	\end{subfigure}
...
\end{figure}
%
\section[\color{red} Analiza nr 2]{Analiza nr 2}
\subsubsection{Dane wejściowe i funkcja straty}
%
...
%
Wykorzystana podczas nauki drugiego modelu funkcja straty $\chi^2$ (\ref{eq_loss2}) jest modyfikacją funkcji wykorzystanej w pierwszej analizie. Do użytej wcześniej funkcji błędu dodany został składnik (\ref{eq_rat}) uwzględniający błąd estymacji stosunku $G_{E_p}/G_{M_p}$.
%
\begin{equation}
...
\section[\color{red} Analiza nr 3]{Analiza nr 3}
\subsubsection{Dane wejściowe i funkcja straty}
%
...
\begin{SCfigure}
	...
	\resizebox{7.cm}{!}{\begin{tikzpicture}
	    ...
	    \node[cnode=white,label=0:$\Delta C_2$] (s-2) at (9,-9) {};
...
\begin{figure}[hp!]
	\centering
	\includegraphics[width=0.98\textwidth]{anl_3.eps}
	\caption{Magnetyczna i elektryczna funkcja postaci. Linia ciągła oznacza średnią ze wszystkich modeli, zacienione pola wyznaczają obszary $1 \sigma$ (jasny) oraz $2\sigma$ (ciemny). Linia przerywana przedstawia wyniki z publikacji \cite{2009PhRvC..79f5204A}. } 
	\label{fig:gm3}
\end{figure}
...
\begin{figure}[hp!]
	\centering
	\includegraphics[width=0.98\textwidth]{ratio_3.eps}
	\caption{Pomiary stosunku elektrycznej i magnetycznej funkcji postaci wraz z dopasowaniem liniowym oraz dopasowaniem modelu statystycznego. Linia ciągła oznacza średni wynik ze wszystkich wytrenowanych modeli, zacienione pola wyznaczają obszary $1 \sigma$ oraz $2\sigma$.} 
	\label{fig:ratio3}
\end{figure}
...
\begin{figure}[hp!]
	\centering
	\includegraphics[width=1\textwidth]{c2_eps.eps}
	\caption{Pomiary stosunku elektrycznej i magnetycznej funkcji postaci wraz z dopasowaniem liniowym oraz dopasowaniem modelu statystycznego. Linia ciągła oznacza średni wynik ze wszystkich wytrenowanych modeli, zacienione pola wyznaczają obszary $1 \sigma$ oraz $2\sigma$.} 
	\label{fig:c2}
\end{figure}
