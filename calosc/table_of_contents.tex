...
\subsection{Dane wejściowe i funkcja straty}
%
%opis = https://arxiv.org/pdf/0812.3539.pdf
%dane = https://arxiv.org/pdf/1106.1204.pdf
%
...
%
%
Funkcja straty to z definicji funkcja przyporządkowująca nieujemną wielkość kary poprzez porównanie zmiennej objaśnianej do wyliczonego estymatora. W modelach regresyjnych bardzo często jako funkcję straty stosuje się błąd średnio kwadratowy (\textit{MSE}) zdefiniowany jako
%
\begin{equation}
\operatorname {MSE} = \frac{1}{n} \sum_{i=1}^{n} \left({\hat {Y_i }}-Y_i \right)^{2}.
\end{equation}
%
W przedstawionym modelu, wykorzystana została zmodyfikowana postać funkcji chi-kwadrat, która bierze pod uwagę nipewność pomiarową oraz systematyczną
%
%\begin{equation}
%\chi_{\sigma}^2 + \chi_{PT}^2 + \chi_{\pm}^2 + \chi_{G_M}^2 + \chi_{G_E}^2    
%\end{equation}
%
\begin{equation}
\chi_{\sigma}^2 = \frac{1}{n}  \sum_{k=1}^{N_{\sigma}} \left[ \sum_{i=1}^{n_k} \left( \frac{\eta_k \sigma^{th}_{ki} - \sigma^{ex}_{ki}}{\Delta \sigma_{ki}} \right)^2 + \left(\frac{\eta_k - 1}{\eta_k}\right)^2 \right],
\end{equation}
%
