...
\begin{itemize}
	\item 4.4 porownanie wynikow
	\item zastanowic się czy wstępu nie przeniesc do fizyki zjawiska
	...
	\item spis treści tak aby zmieścił się na jednej stronie
\end{itemize}
...
całkując po całej objętości protonu, $\vec{Q}$ to przekaz czteropędu podczas rozpraszania. Wyróżniamy elektryczną $G_{E_p}$ i magnetyczną $G_{M_p}$ funkcje postaci, które dla niewielkich wartości $\vec{Q}$ mogą być utożsamiane z transformatami Fouriera gęstości ładunku elektrycznego i prądu magnetyzacji wewnątrz protonu. Ponadto rozwinięcie w szereg dla niewielkich $Q^2$ daje:

\begin{equation}
G(Q^2) = 1 - \frac{1}{6}\langle r^2 \rangle Q^2 + \dots,
\end{equation}
%
gdzie $\langle r^2 \rangle$ to kwadrat średniej kwadratowej promienia protonu. Znajomość funkcji postaci pozwala przy pomocy poniższego równania na oszacowanie promienia protonu czyli jednej z ciągle wątpliwie poznanych właśności cząstki.
...
\end{equation}
%
To wartość o 4\% mniejsza od rekomendowanej przez CODATA (\ref{r_codata}), będąca rozbieżnością na poziomie $7\sigma$.
	
	
Celem tej pracy magisterskiej jest zbudowanie modelu statystycznego, który wykorzystując pomiary eksperymentalne rozpraszania elektron-proton da w wyniku przewidywanie elektrycznej i magnetycznej funkcji postaci. Zainspirowany poprzednimi badaniami \cite{2009PhRvC..79f5204A}, \cite{PhysRevC.84.034314} wykorzystam do tego celu popularny typ sztucznych sieci neuronowych - perceptron wielowarstwowy. Sieci neuronowe są przykładem systemów uczących się, i znalazły szerokie zastosowanie w problemach klasyfikacyjnych oraz regresyjnych. Swoją popularność zawdzięczają biologicznym analogią, ich konstrukcja inspirowana jest budową znajdujących się w mózgu struktur stworzonych przez naturalne neurony i łączące je synapsy. 

Następny rozdział to wprowadzenie do tematyki sieci neuronowych, przedstawiona jest w nim ich historia, struktura i sposób działania. Ponadto przedstawię działanie twierdzenia, które mówi o tym, że sieć neuronowa może realizować aproksymację z dowolnie małym błędem. Rozdział 3 to wprowadzenie do metodologii budowy i nauki modelu statystycznego wraz z omówieniem fizyki zjawiska rozpraszania elektron-proton. W rozdziale 4 zostaną przedstawione wyniki trzech analiz wynikające z wykorzystania różnych technik modelowania oraz rodzajów pomiarów eksperymentalnych. Uzyskane wyniki zostaną porównane z rezultatami przedstawionymi w pracy \cite{PhysRevC.84.034314}. Praca kończy się podsumowaniem w rozdziale 5.
...
\begin{figure}[htp!]
	\centering
	\includegraphics[width=1.\textwidth]{Epkin_recoil_colour}
...
\begin{figure}[htp!]
	\centering
	\includegraphics[width=1.\textwidth]{tpe}
...
\begin{figure}[hp!]
	\centering
	\includegraphics[width=1.2\textwidth]{anl_3.eps}
	\caption{Magnetyczna i elektryczna funkcja postaci. Linia ciągła oznacza średnią ze wszystkich modeli, zacienione pola wyznaczają obszary $1 \sigma$ (jasny) oraz $2\sigma$ (ciemny). Linia przerywana przedstawia wyniki z publikacji \cite{2009PhRvC..79f5204A}. } 
	\label{fig:ff3}
\end{figure}
